...
class YahooNewsScraperInput(BaseModel):
    ticker: str = Field(..., description="The stock ticker symbol to search news for (e.g., AAPL, TSLA)")
    max_articles: int = Field(default=10, description="Maximum number of articles to scrape")
...
class YahooNewsScraperTool(BaseTool):
    ...
    async def _scrape_detailed_page(self, url: str) -> tuple[Optional[str], Optional[str]]:
        ...
        try:
            ...
            # Extract title
            title_elem = soup.find(name=("div", "h1"), class_=lambda x: x and "cover-title" in x)
    ...
    async def _scrape_news_async(self, ticker: str, max_articles: int = 10) -> str:
        ...
        try:
            ...
            articles_data = soup.find_all("li", class_=lambda x: x and "story-item" in x)
            ...
            for article in articles_data:
                ...
                try:
                    ...
                    if not detailed_content:
                        ...
                        detailed_content = summary_elem.get_text(strip=True) if summary_elem else ""
                    
                    # Extract source and published date
                    source = "Yahoo Finance"
                    published_at = "Unknown"
                    
                    source_date_elem = article.find("div", class_=lambda x: x and "publishing" in x)
                    if source_date_elem and len(source_date_elem.contents) >= 3:
                        try:
                            source = source_date_elem.contents[0].get_text(strip=True)
                            published_at = source_date_elem.contents[2].get_text(strip=True)
                    ...
                    news_data.append({
                        'title': title,
                        'content': detailed_content[:500] + "..." if len(detailed_content) > 500 else detailed_content,
                        'url': url,
                        'source': source,
    ...
    def _run(self, ticker: str, max_articles: int = 10) -> str:
        """
        ...
        try:
            ...
            result = loop.run_until_complete(self._scrape_news_async(ticker, max_articles))
            loop.close()
            return result
        except Exception as e:
            logging.error(f"Error in sync wrapper: {e}")
            return f"Error running Yahoo News scraper for {ticker}: {str(e)}"
